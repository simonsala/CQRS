...
{
    [TestFixture]
    public class SQliteEventSourceTest
    {
        private SqliteEventSource _sqliteEventSource;
        ...
        public void Init()
        {
            ...
            _sqliteEventSource = new SqliteEventSource(_config.SqliteEventSource);
        }

        [TearDown]
        public void CleanUp()
        {
            _sqliteEventSource.RemoveEventSourcing();
            _sqliteEventSource.ScaffoldEventSourcing();
        }
        ...
        public void Should_CommitEvent_Successfully()
        {
            ...
            //Act
            _sqliteEventSource.CommitEvent(aggregateQualifiedName, 0, createInventory);
            _sqliteEventSource.CommitEvent(aggregateQualifiedName, 1, addProduct1);
            _sqliteEventSource.CommitEvent(aggregateQualifiedName, 2, addProduct2);
            _sqliteEventSource.CommitEvent(aggregateQualifiedName, 3, removeProduct);
            _sqliteEventSource.CommitEvent(aggregateQualifiedName, 4, updateProduct);

            var eventSources = _sqliteEventSource.GetEventSources(aggregateId);
        ...
        {
            //Act
            var tableExists = _sqliteEventSource.TablesExists();
